...
async function watchAd() { /* ... same as previous version, ensure addLoadingState is used correctly ... */ const watchAdBtn=document.getElementById('watch-ad-btn');const watchAdText=document.getElementById('watch-ad-text');const originalButtonText=watchAdText.innerHTML;watchAdText.innerHTML='<span class="loading"></span> Loading Ad...';watchAdBtn.disabled=true;try{if(typeof window.show_9388838==='function'){window.show_9388838();await new Promise(resolve=>setTimeout(resolve,5000))}else{console.warn("Monetag SDK function not found, simulating ad.");await new Promise(resolve=>setTimeout(resolve,2000))}const earnings=2.0;userStats.totalEarnings+=earnings;userStats.lifetimeEarnings+=earnings;userStats.totalViews+=1;const now=new Date();const todayStr=now.toISOString().split('T')[0];if(userStats.lastAdWatch){const lastWatchDateStr=new Date(userStats.lastAdWatch).toISOString().split('T')[0];if(lastWatchDateStr!==todayStr)userStats.todayViews=0}else userStats.todayViews=0;userStats.todayViews+=1;if(userStats.lastAdWatch){const lastWatchDate=new Date(userStats.lastAdWatch);const lastDay=new Date(lastWatchDate.getFullYear(),lastWatchDate.getMonth(),lastWatchDate.getDate());const currentDay=new Date(now.getFullYear(),now.getMonth(),now.getDate());const diffTime=currentDay.getTime()-lastDay.getTime();const diffDays=Math.round(diffTime/(1000*60*60*24));if(diffDays===1){userStats.streakDays+=1}else if(diffDays>1){userStats.streakDays=1}}else{userStats.streakDays=1}userStats.lastAdWatch=now.toISOString();if(!userStats.adHistory)userStats.adHistory=[];userStats.adHistory.push({timestamp:now.toISOString(),earnings:earnings});if(userStats.adHistory.length>20)userStats.adHistory.shift();await saveUserData();updateDashboard();showToast('Ad completed!',`You've earned $${earnings.toFixed(3)}.`, 'success')}catch(error){console.error("Error watching ad:",error);showToast('Ad failed',error.message||'Failed to watch ad.','error')}finally{watchAdText.innerHTML=originalButtonText;watchAdBtn.disabled=false}}
